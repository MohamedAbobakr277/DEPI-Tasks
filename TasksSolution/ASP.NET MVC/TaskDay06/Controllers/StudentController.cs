...
namespace EFCoreTask.Controllers
{
    public class StudentController : Controller
    {
        ...
        {
            var stuCrsRes = _context.StuCrsRes
            ...
            if (stuCrsRes == null) 
            {
                var existingResults = _context.StuCrsRes.Select(s => $"StudentId: {s.StudentId}, CourseId: {s.CourseId}").ToList();
                string message = $"No grade found for StudentId={studentId} and CourseId={courseId}.\n";
                if (existingResults.Any())
                {
                    message += "Valid combinations in your database are:\n" + string.Join("\n", existingResults);
                }
                else
                {
                    message += "You have NO grades in the StuCrsRes table! Please add a grade for a student first.";
                }
                return Content(message, "text/plain");
            }

            string displayColor = stuCrsRes.Grade >= stuCrsRes.Course.MinDegree ? "green" : "red";

            var viewModel = new EFCoreTask.ViewModels.StudentCourseResultViewModel
            {
                StudentName = stuCrsRes.Student.Name,
                CourseName = stuCrsRes.Course.Name,
                Grade = stuCrsRes.Grade,
